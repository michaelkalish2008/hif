...
The **branch field** restores the shape of the trajectory's stochastic branch cloud that a single continuity scalar reduces to one number — the *sampling-perturbation* twin of the perturbation field, in embedding (geometric) space. From the branch embeddings: `n_branches`, per-branch **mean_radius / radius_variance / max_radius**, and a **cluster_count**. cluster_count is the **multi-modality** signal — ≥ 2 means the branches split into distinct semantic modes (genuinely divergent futures a mean similarity hides). Radii are cosine distances in `[0, 2]`.

A **field_dispersion** was here until schema 0.15.0, computed as `1 − mean pairwise cosine` over the same embeddings `trajectory_continuity` is computed from, by the same function — so the two summed to exactly 1.0 in all 120 published profiles. That is the `1 − x` shape the measurement set dropped in favour of raw expressions, surviving one layer down where the Significance Gate does not reach. The raw expression, `trajectory_continuity`, ships in the same block; the complement is a reading a consumer computes, not a second number to store. `PerturbationField.field_dispersion` is a different quantity and is untouched — a raw generalized JSD in bits, not a complement of anything. Persisted as `trajectory.branch_field`; `None` with fewer than two branches or a skipped trajectory stage.
